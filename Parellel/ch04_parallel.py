...
async def run_llm_parallel(prompt_details):
    ...
    tasks = [ # 각 LLM 호출을 coroutine 형태의 작업(task) 리스트로 생성 
        llm_call_async(prompt['user_prompt'], prompt['model']) # llm aou 호출 1개 비동식
...
# 메인 비동기 함수
async def main(): #main에서 사용해야지, await 사용 가능하다 (asyn안하면 불가능)
